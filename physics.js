let physics = {
  update(gameObj) {
    ...
    this.bgCollision(gameObj); //checks collision to the background objects
    ...
    scenery.forEach((scene) => {
      if (this.checkRectCollision(scene, mario)) {
        ...
        if (scene.type == "pipe" || scene.type == "stair") {
          ...
          if (
            mario.posY < scene.posY &&
            mario.posX + mario.width > scene.posX &&
            scene.posX + scene.posY > mario.posX &&
            mario.velY >= 0
          ) {
